...
export function buildGenerationPrompt(
  ...
  if (hasSourcePhoto) {
    parts.push(
      "任务：把输入照片中的孩子变回TA三四岁婴幼儿时期的样子，做成可爱的大头贴贴纸。" +
        ...
        "构图（与照片无关）：纯白背景上一个剪出来的悬浮头部，端正朝前、完整居中不被画面边缘裁切，下巴边缘即内容结束，绝无脖子、肩膀、衣物；" +
        "照片里的图标、按钮、文字、水印等非人物元素一律忽略，绝不出现在画面里。" +
  ...
  skin +=
    "，皮肤细腻有真实儿童肌理。肤色呈现规则：浅色和中等肤色（瓷白/白皙/浅麦/小麦色系）要明显提亮，" +
    "呈现白皙干净、通透发光的幼儿肌肤；深色肤色（蜜糖色/棕色/深棕色，如非裔、拉美裔）保持本来的基调不提亮，" +
    "只增加健康的光泽感。背景必须是纯白色。";
  parts.push(skin);
  ...
  parts.push(
    "整体氛围（重点）：软萌可爱的幼儿感——柔软带绒毛感的发丝、柔和的奶萌气质；" +
      "神态必须天真清澈，像小孩子那样憨和真诚，绝不能有成熟感、大人感或妩媚感，不摆姿势不抛媚眼，头部端正朝前；" +
